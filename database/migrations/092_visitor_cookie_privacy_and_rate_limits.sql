...
-- Remove durable network/device identity while preserving immutable claims,
-- visitor ordinals, choices, and aggregate counters.
DROP FUNCTION IF EXISTS public.axora_claim_public_visitor_fallback(
  text,text,text,text,text,text
);
DROP FUNCTION IF EXISTS public.axora_claim_public_visitor(
  text,text,text,text,text,text,text,timestamptz,text
);
DROP FUNCTION IF EXISTS public.axora_public_visitor_snapshot_v2(
  text,text,text,text
);
DROP FUNCTION IF EXISTS public.axora_public_visitor_snapshot(
  text,text,text
);

DROP TABLE IF EXISTS public.public_visitor_network_claims;

ALTER TABLE public.public_visitor_claims
  DROP COLUMN IF EXISTS network_hash,
  DROP COLUMN IF EXISTS network_device_hash,
  DROP COLUMN IF EXISTS client_signal_hash,
  DROP COLUMN IF EXISTS turnstile_device_hash;

-- Old visitor buckets used a stable network HMAC. They are abuse-control
-- artifacts, not historical claim evidence, so remove them during migration.
...
BEGIN
  ...
  RETURN QUERY
  SELECT state.total_count,state.total_count,state.early_bird_count,
    state.night_owl_count,matched_number,matched_choice
  FROM public.public_visitor_counter_state state
  WHERE state.singleton=true;
END $$;

CREATE OR REPLACE FUNCTION public.axora_claim_public_visitor_v3(
  p_token_hash text,
  p_choice text,
  p_locale text,
...
BEGIN
  IF p_token_hash IS NULL OR p_token_hash !~ '^[0-9a-f]{64}$' THEN
    RAISE EXCEPTION 'Public visitor token fingerprint is invalid';
  END IF;
  IF p_choice NOT IN ('EARLY_BIRD','NIGHT_OWL')
    OR p_locale NOT IN ('en','ar','ms') THEN
  ...
  INSERT INTO public.public_visitor_claims(
    visitor_number,choice,locale,turnstile_challenge_at,
    turnstile_hostname,turnstile_action,verification_method
  ) VALUES (
    next_total,p_choice,p_locale,p_turnstile_challenge_at,
  ...
  RETURN QUERY
  SELECT next_total,next_total,next_early,next_night,
