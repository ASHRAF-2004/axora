  ...
  return <><PageHeader eyebrow="Supplier control" title="Sourcing and quotations" description="Capture comparable supplier offers, then select one with a written reason. The selected price becomes the request line buying price." />
    {canManage ? <section className="detail-grid"><article className="panel form-panel"><h2>Add a quotation</h2><p>Use the supplier&apos;s real written quotation. Do not guess a reference or price.</p>
      <form action={createQuotationAction}><div className="form-grid">
        <label className="field-full">Request line<select name="requestLineId" required defaultValue=""><option value="" disabled>Select line</option>{lines.map((line) => <option key={line.id} value={line.id}>{line.companyName} · {line.orderCode} · {line.code} · {line.productName}</option>)}</select></label>
        <label>Supplier<select name="supplierId" required defaultValue=""><option value="" disabled>Select supplier</option>{suppliers.filter((item) => item.status === "Active").map((item) => <option key={item.id} value={item.id}>{actor.isOwner ? `${item.companyName || "Unassigned"} · ` : ""}{item.code} · {item.name}</option>)}</select></label>
