...
export async function deleteOrArchiveCompanyAction(companyId: string, formData: FormData) {
  ...
  const input = z.object({ companyId: z.uuid(), confirmation: z.string().min(8).max(100), reason: z.string().trim().min(3).max(1000) }).parse({ companyId, confirmation: formData.get("confirmation"), reason: formData.get("reason") });
