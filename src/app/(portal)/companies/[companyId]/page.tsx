...
export default async function CompanyDetailPage({ params }: { params: Promise<{ companyId: string }> }) {
  ...
  return <>
    ...
    {impact ? <section className="panel"><h2>{local.deletion}</h2><p>{impact.protectedEvidence > 0 ? local.protected : impact.hardDeleteEligible ? local.hard : local.archive}</p><dl className="summary-list">{[
      ...
      [local.email, impact.pendingWorkflowEmails], [local.inFlight, impact.inFlightWork], [local.evidence, impact.protectedEvidence],
    ].map(([label, count]) => <div key={label}><dt>{label}</dt><dd>{count}</dd></div>)}</dl><details><summary>{impact.hardDeleteEligible ? local.permanent : local.archiveAction}</summary><form action={deleteOrArchiveCompanyAction.bind(null, company.id)} className="table-action-stack"><label>{local.reason}<textarea name="reason" minLength={3} maxLength={1000} required /></label><label>{local.type} <strong>{impact.confirmation}</strong><input name="confirmation" autoComplete="off" required /></label><button className="button button-secondary" type="submit">{local.confirm}</button></form></details></section> : null}
  </>;
}
