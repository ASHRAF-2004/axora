...
export default async function CompaniesPage() {
  ...
  return <><PageHeader eyebrow="Master data" title="Companies" description="One approved company record connects its branches, requests, invoices and reporting." />
    <section className="split-layout"><article className="panel"><div className="panel-header"><div><h2>Company master</h2><p>{companies.length} company records</p></div></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Code</th><th>Company</th><th>Industry</th><th>Main contact</th><th>Payment terms</th><th>Status</th>{actor.isOwner ? <th>Action</th> : null}</tr></thead><tbody>{companies.map((company) => <tr key={company.id}><td><strong>{company.code}</strong></td><td><strong>{company.name}</strong><br /><span className="subtle">{company.billingCycle}</span></td><td>{company.industry}</td><td>{company.mainContactName}<br /><span className="subtle">{company.mainContactEmail}</span></td><td>{company.paymentTerms}</td><td><StatusBadge>{company.status}</StatusBadge></td>{actor.isOwner ? <td><form action={setMasterActiveAction.bind(null, "companies", company.id, company.status !== "Active")}><button className="button button-secondary" type="submit">{company.status === "Active" ? "Deactivate" : "Activate"}</button></form></td> : null}</tr>)}</tbody></table></div></article>
    {actor.isOwner ? <form action={createCompanyAction} className="panel form-panel"><h2>Add a company</h2><p>Codes are generated automatically and remain stable.</p><div className="form-grid">
    ...
    </div><div className="form-actions"><button className="button button-primary" type="submit">Add company</button></div></form> : null}</section></>;
}
