...
export default async function DriverDetailPage({ params }: { params: Promise<{ driverId: string }> }) {
  ...
  return <>
    ...
    <section className="detail-grid">
      <article className="panel"><h2>{copy.profile}</h2><dl className="summary-list"><div><dt>{copy.contact}</dt><dd>{driver.email}<br />{driver.phone || "—"}</dd></div><div><dt>{copy.state}</dt><dd>{driver.active ? copy.active : copy.deactivated} · {driverAvailabilityLabel(driver.availability, locale)}</dd></div><div><dt>{copy.vehicle}</dt><dd>{driver.vehicle || copy.notRecorded}</dd></div><div><dt>{copy.current}</dt><dd>{activeJob ? `${activeJob.code} · ${deliveryWorkflowStatusLabel(activeJob.status, locale)}` : copy.none}</dd></div><div><dt>{copy.location}</dt><dd>{latest ? `${date(latest.capturedAt)} · ±${Math.round(latest.accuracy)} m` : copy.noLocation}</dd></div></dl></article>
      <article className="panel"><h2>{copy.actions}</h2>{activeJob ? <><p className={recovery?.eligible ? "callout callout-warning" : "callout"}>{recovery ? driverRecoveryReasonLabel(recovery.reasonCode, locale) : copy.blocked}</p><details><summary>{copy.recover}</summary><form action={releaseStuckDeliveryJobAction.bind(null, activeJob.id)} className="table-action-stack"><input type="hidden" name="driverId" value={driver.id} /><input type="hidden" name="commandId" value={randomUUID()} /><button className="button button-secondary" disabled={!recovery?.eligible} type="submit">{copy.release}</button></form></details></> : null}<div className="table-action-stack">{canAccess(actor, "manage_users") ? <Link className="button button-secondary" href={`/users/${driver.id}/access`}>{people.openAccess}</Link> : null}<form action={requestDriverPasswordResetAction.bind(null, driver.id)}><button className="button button-secondary" type="submit">{copy.reset}</button></form><form action={setUserActiveAction.bind(null, driver.id, !driver.active)}><button className="button button-secondary" disabled={Boolean(activeJob)} type="submit">{driver.active ? copy.deactivate : copy.activate}</button></form><details><summary>{copy.delete}</summary><form action={removeUserAction.bind(null, driver.id)} className="table-action-stack"><label><input name="confirmRemoval" type="checkbox" value="confirmed" required /> {copy.confirm}</label><button className="button button-secondary" disabled={Boolean(activeJob)} type="submit">{copy.delete}</button></form></details></div></article>
    </section>
