...
export default async function DeliveriesPage() {
  ...
  const lines = requests.flatMap((request) => request.lines.map((line) => ({ ...line, orderCode: request.orderCode, companyName: request.companyName })));
  return <><PageHeader eyebrow="Receiving control" title="Deliveries" description="Create one record for every shipment. Partial deliveries stay separate, so received quantities can be checked against the original order." />
    <section className="panel form-panel"><h2>Add a delivery update</h2><form action={recordDeliveryAction}><div className="form-grid">
