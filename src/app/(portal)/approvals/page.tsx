...
export default async function ApprovalsPage() {
  ...
  const canManage = hasPermission(actor.role, "manage_approvals");
  ...
  return <><PageHeader eyebrow="Supervisor evidence" title="Approvals" description="Record who reviewed a request, the decision, and the reason. A rejected decision always needs an explanation." />
    {canManage ? <section className="detail-grid"><article className="panel form-panel"><h2>Record a decision</h2><form action={recordApprovalAction}><div className="form-grid">
      <label className="field-full">Request<select name="requestId" required defaultValue=""><option value="" disabled>Select request</option>{requests.filter((item) => item.status === "Waiting for Approval").map((item) => <option key={item.id} value={item.id}>{item.orderCode} · {item.companyName} · {item.status}</option>)}</select></label>
