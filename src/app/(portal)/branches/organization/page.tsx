...
import { DeferredOrganizationActions } from "@/components/DeferredOrganizationActions";
...
function StatusForm({ nodeType, nodeId, active, copy }: {
  ...
  return <DeferredOrganizationActions label={copy.update}><form action={setOrganizationNodeActiveAction} className="table-action-stack">
  ...
  </form></DeferredOrganizationActions>;
}
...
export default async function OrganizationPage({
  ...
  return <>
    ...
    <div className="action-row organization-back-row"><Link className="button button-secondary" href="/branches">{copy.back}</Link></div>
    {notice ? <section className="panel" role="status" aria-live="polite"><strong>{notice}</strong></section> : null}

    <section className="panel organization-hierarchy-panel"><div className="panel-header"><div><h2>{copy.hierarchy}</h2><p>{copy.hierarchyHelp}</p></div></div>
      <div className="data-table-wrap"><table className="data-table"><thead><tr><th>{copy.branches}</th><th>{copy.company}</th><th>{copy.timezone}</th><th>{copy.active}</th></tr></thead><tbody>{workspace.branches.map((branch) => <tr key={branch.id}><td><strong>{branch.name}</strong><br /><span className="subtle">{branch.branchCode}</span></td><td>{branch.companyName}</td><td>{branch.timezone}</td><td><StatusBadge status={branch.status}>{branch.status === "Active" ? copy.active : copy.inactive}</StatusBadge></td></tr>)}</tbody></table></div>
    </section>

    {workspace.canManageBranches && workspace.branches.length ? <DeferredOrganizationActions label={copy.update}><section className="detail-grid organization-branch-actions">{workspace.branches.map((branch) => <article className="panel form-panel" key={branch.id}><div className="panel-header"><div><h2>{branch.name}</h2><p>{copy.update}</p></div><StatusBadge status={branch.status}>{branch.status === "Active" ? copy.active : copy.inactive}</StatusBadge></div><form action={saveOrganizationNodeAction} className="table-action-stack"><input type="hidden" name="nodeType" value="BRANCH" /><input type="hidden" name="nodeId" value={branch.id} /><input type="hidden" name="companyId" value={branch.companyId} /><input type="hidden" name="code" value={branch.branchCode} /><label>{copy.name}<input name="name" defaultValue={branch.name} required /></label><label>{copy.timezone}<select name="timezone" defaultValue={branch.timezone}>{TIMEZONES.map((zone) => <option key={zone}>{zone}</option>)}</select></label><button className="button button-secondary" type="submit">{copy.update}</button></form><StatusForm copy={copy} nodeType="BRANCH" nodeId={branch.id} active={branch.status === "Active"} /></article>)}</section></DeferredOrganizationActions> : null}

    <section className="detail-grid">
      <article className="panel"><h2>{copy.departments}</h2>{workspace.departments.length ? <ul className="list-reset">{workspace.departments.map((department) => <li key={department.id} className="callout organization-tree-node" data-depth={depth(department.id, departmentParents)}><div className="panel-header"><div><strong>{department.name}</strong><br /><span className="subtle">{department.code} · {branchName.get(department.branchId ?? "") ?? companyName.get(department.companyId)}</span></div><StatusBadge>{department.active ? copy.active : copy.inactive}</StatusBadge></div>{workspace.canManageDepartments ? <StatusForm copy={copy} nodeType="DEPARTMENT" nodeId={department.id} active={department.active} /> : null}</li>)}</ul> : <p className="subtle">{copy.noRecords}</p>}</article>
      <article className="panel"><h2>{copy.businessUnits}</h2>{workspace.businessUnits.length ? <ul className="list-reset">{workspace.businessUnits.map((unit) => <li key={unit.id} className="callout organization-tree-node" data-depth={depth(unit.id, unitParents)}><div className="panel-header"><div><strong>{unit.name}</strong><br /><span className="subtle">{unit.code} · {companyName.get(unit.companyId)}</span></div><StatusBadge>{unit.active ? copy.active : copy.inactive}</StatusBadge></div>{workspace.canManageCostCentres ? <StatusForm copy={copy} nodeType="BUSINESS_UNIT" nodeId={unit.id} active={unit.active} /> : null}</li>)}</ul> : <p className="subtle">{copy.noRecords}</p>}</article>
    </section>

    {(workspace.canManageDepartments || workspace.canManageCostCentres) ? <DeferredOrganizationActions label={copy.update}><section className="detail-grid">
      {workspace.canManageDepartments ? <article className="panel form-panel"><h2>{copy.createDepartment}</h2><form action={saveOrganizationNodeAction} className="form-grid"><input type="hidden" name="nodeType" value="DEPARTMENT" /><label>{copy.company}<select name="companyId" required>{companyOptions.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label><label>{copy.branch}<select name="branchId"><option value="">{copy.noDepartment}</option>{workspace.branches.filter((branch) => branch.status === "Active").map((branch) => <option key={branch.id} value={branch.id}>{branch.companyName} · {branch.name}</option>)}</select></label><label>{copy.parent}<select name="parentId"><option value="">{copy.noParent}</option>{workspace.departments.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{companyName.get(item.companyId)} · {item.name}</option>)}</select></label><label>{copy.code}<input name="code" required pattern="[A-Za-z0-9][A-Za-z0-9_-]{1,39}" /></label><label>{copy.name}<input name="name" required /></label><label>{copy.timezone}<select name="timezone" defaultValue="Asia/Kuala_Lumpur">{TIMEZONES.map((zone) => <option key={zone}>{zone}</option>)}</select></label><label className="field-full">{copy.descriptionLabel}<textarea name="description" maxLength={1000} /></label><div className="form-actions field-full"><button className="button button-primary" type="submit">{copy.create}</button></div></form></article> : null}
      {workspace.canManageCostCentres ? <article className="panel form-panel"><h2>{copy.createBusinessUnit}</h2><form action={saveOrganizationNodeAction} className="form-grid"><input type="hidden" name="nodeType" value="BUSINESS_UNIT" /><label>{copy.company}<select name="companyId" required>{companyOptions.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label><label>{copy.parent}<select name="parentId"><option value="">{copy.noParent}</option>{workspace.businessUnits.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{companyName.get(item.companyId)} · {item.name}</option>)}</select></label><label>{copy.code}<input name="code" required pattern="[A-Za-z0-9][A-Za-z0-9_-]{1,39}" /></label><label>{copy.name}<input name="name" required /></label><label className="field-full">{copy.descriptionLabel}<textarea name="description" maxLength={1000} /></label><div className="form-actions field-full"><button className="button button-primary" type="submit">{copy.create}</button></div></form></article> : null}
      {workspace.canManageCostCentres ? <article className="panel form-panel"><h2>{copy.createCostCentre}</h2><form action={saveOrganizationNodeAction} className="form-grid"><input type="hidden" name="nodeType" value="COST_CENTRE" /><label>{copy.company}<select name="companyId" required>{companyOptions.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label><label>{copy.businessUnit}<select name="businessUnitId"><option value="">{copy.noParent}</option>{workspace.businessUnits.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{companyName.get(item.companyId)} · {item.name}</option>)}</select></label><label>{copy.branch}<select name="branchId"><option value="">{copy.noDepartment}</option>{workspace.branches.filter((branch) => branch.status === "Active").map((branch) => <option key={branch.id} value={branch.id}>{branch.companyName} · {branch.name}</option>)}</select></label><label>{copy.department}<select name="departmentId"><option value="">{copy.noDepartment}</option>{workspace.departments.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{companyName.get(item.companyId)} · {item.name}</option>)}</select></label><label>{copy.code}<input name="code" required /></label><label>{copy.name}<input name="name" required /></label><label>{copy.currency}<input name="currency" defaultValue="MYR" required pattern="[A-Za-z]{3}" /></label><label className="field-full">{copy.descriptionLabel}<textarea name="description" /></label><div className="form-actions field-full"><button className="button button-primary" type="submit">{copy.create}</button></div></form></article> : null}
    </section></DeferredOrganizationActions> : null}
