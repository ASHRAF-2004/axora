...
export default async function OrganizationPage({
  ...
  return <>
    ...
    <section className="panel organization-hierarchy-panel"><div className="panel-header"><div><h2>{copy.hierarchy}</h2><p>{copy.hierarchyHelp}</p></div></div>
      <div className="data-table-wrap"><table className="data-table"><thead><tr><th>{copy.branches}</th><th>{copy.company}</th><th>{copy.timezone}</th><th>{copy.active}</th>{workspace.canManageBranches ? <th>{copy.update}</th> : null}</tr></thead><tbody>{workspace.branches.map((branch) => <tr key={branch.id}><td><strong>{branch.name}</strong><br /><span className="subtle">{branch.branchCode}</span></td><td>{branch.companyName}</td><td>{branch.timezone}</td><td><StatusBadge status={branch.status}>{branch.status === "Active" ? copy.active : copy.inactive}</StatusBadge></td>{workspace.canManageBranches ? <td><details><summary>{copy.edit}</summary><div><form action={saveOrganizationNodeAction} className="table-action-stack"><input type="hidden" name="nodeType" value="BRANCH" /><input type="hidden" name="nodeId" value={branch.id} /><input type="hidden" name="companyId" value={branch.companyId} /><input type="hidden" name="code" value={branch.branchCode} /><label>{copy.name}<input name="name" defaultValue={branch.name} required /></label><label>{copy.timezone}<select name="timezone" defaultValue={branch.timezone}>{TIMEZONES.map((zone) => <option key={zone}>{zone}</option>)}</select></label><label>{copy.reason}<input name="reason" required minLength={3} /></label><button className="button button-secondary" type="submit">{copy.update}</button></form></div></details><StatusForm copy={copy} nodeType="BRANCH" nodeId={branch.id} active={branch.status === "Active"} /></td> : null}</tr>)}</tbody></table></div>
    </section>
