...
export default async function BranchesPage() {
  const [branches, companies] = await Promise.all([listBranches(), listCompanies()]);
  return <><PageHeader eyebrow="Master data" title="Branches" description="Every branch belongs to one company and carries its own delivery address and contact instructions." />
    <section className="split-layout"><article className="panel"><div className="panel-header"><div><h2>Branch master</h2><p>{branches.length} branch records</p></div></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>Code</th><th>Branch</th><th>Company</th><th>Area</th><th>Contact</th><th>Status</th><th>Action</th></tr></thead><tbody>{branches.map((branch) => <tr key={branch.id}><td><strong>{branch.code}</strong><br /><span className="subtle">{branch.branchCode}</span></td><td><strong>{branch.name}</strong><br /><span className="subtle">{branch.deliveryAddress}</span></td><td>{branch.companyName}</td><td>{branch.city}</td><td>{branch.contactName}<br /><span className="subtle">{branch.contactPhone}</span></td><td><StatusBadge>{branch.status}</StatusBadge></td><td><form action={setMasterActiveAction.bind(null, "branches", branch.id, branch.status !== "Active")}><button className="button button-secondary" type="submit">{branch.status === "Active" ? "Deactivate" : "Activate"}</button></form></td></tr>)}</tbody></table></div></article>
    <form action={createBranchAction} className="panel form-panel"><h2>Add a branch</h2><p>Choose the parent company instead of typing it again.</p><div className="form-grid">
      <label className="field-full">Company<select name="companyId" required defaultValue=""><option value="" disabled>Select company</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.code} · {company.name}</option>)}</select></label>
    ...
    </div><div className="form-actions"><button className="button button-primary" type="submit">Add branch</button></div></form></section></>;
}
