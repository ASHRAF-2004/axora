...
export default async function NewUserPage() {
  ...
  const [organization, structure, permissionOptions] = await Promise.all([
  ...
  return <>
    ...
    <section className="detail-grid">
      <article className="panel form-panel">
        <UserCreateForm actorBranchId={actor.branchId} actorCompanyId={actor.companyId} actorDepartmentId={actor.departmentId} actorIsOwner={actor.isOwner} defaultLocale={locale} branches={branches} companies={companies} departments={structure.departments} permissionOptions={permissionOptions} roleOptions={roles.map((role) => ({ value: role.key, label: role.label, description: role.description, category: role.category, accountKind: role.accountKind, allowedScopes: role.allowedScopes, defaultPermissions: defaultPermissionsForRole(role.key, role.allowedScopes[0], role.key === "PLATFORM_OWNER") }))} />
      </article>
