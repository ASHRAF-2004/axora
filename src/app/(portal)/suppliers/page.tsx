...
export default async function SuppliersPage() {
  ...
  return <><PageHeader eyebrow="Master data" title="Suppliers" description="Store approved supplier contacts, coverage, lead time, payment terms and product capability once." />
    ...
    <form action={createSupplierAction} className="panel form-panel"><h2>Add a supplier</h2><p>Do not create a duplicate when the supplier already exists.</p><div className="form-grid">
      {actor.isOwner ? <label className="field-full">Company<select name="companyId" required defaultValue=""><option value="" disabled>Select company</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.code} · {company.name}</option>)}</select></label> : null}
