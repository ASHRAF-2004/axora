...
export default async function RequestsPage({
  ...
  const detailCopy=requestDetailMessages(locale);
  const cartCopy=cartMessages(locale);
  const platformView=actor.isOwner || actor.accountKind==="PLATFORM";
  const companyAdministrator=usesCompanyAdministratorDirectPurchase(actor);
  ...
  const hasDirectOrders=requests.some((request) => request.purchaseMode==="COMPANY_ADMIN_DIRECT");
  ...
  return (
    <>
      <PageHeader
        ...
        actionHref={canDirectPurchase
          ? "/products"
          : !companyAdministrator && canAccess(actor,"create_requests")
            ? "/requests/new" : undefined}
        actionLabel={canDirectPurchase
          ? cartCopy.placeOrder
          : !companyAdministrator && canAccess(actor,"create_requests")
            ? copy.create : undefined}
      />
      <RequestFiltersPanel
      ...
      <section className="panel" id="request-table">
        <div className="data-table-wrap"><table className="data-table">
          <thead><tr><th>{copy.request}</th><th>{platformView ? copy.customerBranch : copy.branch}</th><th>{copy.items}</th><th>{copy.neededBy}</th><th>{hasDirectOrders ? detailCopy.approvalOrPayment : copy.approval}</th><th>{copy.fulfilment}</th><th>{platformView ? copy.customerTotal : copy.estimatedTotal}</th><th>{copy.delivery}</th>{canViewInvoices ? <th>{copy.payment}</th> : null}</tr></thead>
          <tbody>{requests.map((request) => {
            ...
            return <tr key={request.id}>
              ...
              <td>{request.purchaseMode==="COMPANY_ADMIN_DIRECT"
                ? <><StatusBadge status="Paid">{localizedStatus("Paid",locale)}</StatusBadge><br /><span className="subtle">{detailCopy.directOrder}</span></>
                : <StatusBadge status={request.approvalStatus}>{localizedStatus(request.approvalStatus,locale)}</StatusBadge>}</td>
