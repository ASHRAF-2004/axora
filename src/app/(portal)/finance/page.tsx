...
export default async function FinancePage() {
  ...
  />
    ...
    <section className="panel" style={{ marginTop: 17 }}><div className="panel-header"><div><h2>{m("finance.paymentRegister")}</h2><p>{m("finance.paymentIntro")}</p></div></div><div className="data-table-wrap"><table className="data-table"><thead><tr><th>{m("finance.invoice")}</th><th>{m("finance.date")}</th><th>{m("finance.amount")}</th><th>{m("finance.reference")}</th><th>{m("finance.recordedBy")}</th></tr></thead><tbody>{payments.map((item) => <tr key={item.id}><td>{item.invoiceNumber}</td><td>{formatDate(item.paymentDate, locale, actor.timezone)}</td><td><strong>{formatCurrency(item.amount, locale)}</strong></td><td>{item.reference || "—"}</td><td>{item.recordedByName || "—"}</td></tr>)}</tbody></table></div></section>
