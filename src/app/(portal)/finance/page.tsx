...
export default async function FinancePage() {
  ...
  return <><PageHeader eyebrow="Finance control" title="Invoices and COD payments" description="The MVP records cash on delivery only as a safety boundary. Axora does not initiate or process online payments; other methods require separately approved work." />
    <section className="dashboard-grid"><article className="panel form-panel"><h2>Add invoice</h2><form action={createInvoiceAction}><div className="form-grid">
      ...
      <label>Request<select name="requestId" required defaultValue=""><option value="" disabled>Select request</option>{requests.map((item) => <option key={item.id} value={item.id}>{item.orderCode} · {item.companyName}</option>)}</select></label>
      <label>Supplier (supplier invoice only)<select name="supplierId" defaultValue=""><option value="">Not applicable</option>{suppliers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
