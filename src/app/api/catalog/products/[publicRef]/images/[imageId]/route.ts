...
export async function GET(request: Request, { params }: { params: Promise<{ publicRef: string; imageId: string }> }) {
  const actor = await getSession();
  if (!actor) return Response.json({ error: "Authentication required" }, { status: 401 });
  if (!canAccess(actor, "view_catalog")) return Response.json({ error: "Catalog access denied" }, { status: 403 });
  const { publicRef, imageId } = await params;
  const product = (await getCatalogProductsByPublicRefs([publicRef], actor))[0];
  const image = product ? await loadProductImage(product.id, actor, imageId) : null;
