...
export async function submitContactAction(locale: string, formData: FormData) {
  ...
  try {
    ...
    await submitPublicContact({
      ...
      employeeRange: String(formData.get("employeeRange") ?? "") as never,
      branchRange: String(formData.get("branchRange") ?? "") as never,
      spendRange: String(formData.get("spendRange") ?? "") as never,
      contactMethod: String(formData.get("contactMethod") ?? "") as never,
      contactTimezone: String(formData.get("contactTimezone") ?? ""),
