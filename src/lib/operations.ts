...
export async function createQuotation(input: NewQuotationInput, actor: { id: string; name: string }) {
  if (isDemoMode()) {
  ...
  await withAuditTransaction({ userId: actor.id }, (client) => client.query(`INSERT INTO quotations
    ...
    VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,lookup_id('quotation_status','Received'))`,
...
export async function selectQuotation(id: string, reason: string, actor: { id: string; name: string }) {
  ...
  await withAuditTransaction({ userId: actor.id, reason }, async (client) => {
    const quotation = await client.query<{ requestLineId: string; supplierId: string; reference: string; unitPrice: number; deliveryCharge: number }>(
      `SELECT request_line_id::text AS "requestLineId",supplier_id::text AS "supplierId",COALESCE(quotation_reference,'') AS reference,unit_price::float8 AS "unitPrice",delivery_charge::float8 AS "deliveryCharge" FROM quotations WHERE id=$1 FOR UPDATE`, [id]);
...
export async function recordApproval(input: { requestId: string; approvalType: string; status: ApprovalRecord["status"]; reason?: string }, actor: { id: string; name: string }) {
  ...
  await withAuditTransaction({ userId: actor.id, reason: input.reason }, (client) => client.query(`INSERT INTO approvals
    (request_id,approval_type,status,reviewer_id,reason,decided_at) VALUES ($1,$2,$3,$4,$5,CASE WHEN $3='Pending' THEN NULL ELSE now() END)`,
    [input.requestId, input.approvalType, input.status, actor.id, input.reason ?? null]));
}
...
export async function listDeliveries(): Promise<DeliveryRecord[]> {
  ...
  const result = await query<DeliveryRecord>(`SELECT d.id::text,d.request_line_id::text AS "requestLineId",l.request_line_code AS "requestLineCode",r.order_code AS "orderCode",
    ...
    JOIN lookup_values st ON st.id=d.status_id ORDER BY d.created_at DESC`);
  return result.rows;
}

export async function recordDelivery(input: { requestLineId: string; expectedDate?: string; revisedDate?: string; actualDate?: string; status: DeliveryStatus; quantityReceived: number; receivedBy?: string; issueReason?: string }, actor: { id: string; name: string }) {
  ...
  await withAuditTransaction({ userId: actor.id, reason: input.issueReason }, async (client) => {
    const line = await client.query<{ quantity: number }>("SELECT quantity::float8 AS quantity FROM request_lines WHERE id=$1 FOR UPDATE", [input.requestLineId]);
...
export async function listInvoices(): Promise<InvoiceRecord[]> {
  ...
  const result = await query<InvoiceRecord>(`SELECT b.id::text,b.direction,b.request_id::text AS "requestId",r.order_code AS "orderCode",
    ...
    ORDER BY b.invoice_date DESC,b.invoice_number`);
  return result.rows;
}

export async function createInvoice(input: { direction: "CUSTOMER" | "SUPPLIER"; requestId: string; supplierId?: string; invoiceNumber: string; invoiceDate: string; dueDate?: string; amount: number; status: InvoiceStatus }, actor: { id: string; name: string }) {
  ...
  await withAuditTransaction({ userId: actor.id }, async (client) => {
    const request = await client.query<{ companyId: string }>("SELECT company_id::text AS \"companyId\" FROM requests WHERE id=$1", [input.requestId]);
...
export async function listPayments(): Promise<PaymentRecord[]> {
  ...
  const result = await query<PaymentRecord>(`SELECT p.id::text,p.invoice_id::text AS "invoiceId",i.invoice_number AS "invoiceNumber",p.payment_date::text AS "paymentDate",
    ...
    LEFT JOIN users u ON u.id=p.recorded_by ORDER BY p.payment_date DESC,p.created_at DESC`);
  return result.rows;
}

export async function recordPayment(input: { invoiceId: string; paymentDate: string; amount: number; method: string; reference?: string }, actor: { id: string; name: string }) {
  ...
  await withAuditTransaction({ userId: actor.id }, async (client) => {
    const invoice = await client.query<{ amount: number }>("SELECT amount::float8 AS amount FROM invoices WHERE id=$1 FOR UPDATE", [input.invoiceId]);
...
export async function listAttachments(): Promise<AttachmentRecord[]> {
  ...
  const result = await query<AttachmentRecord>(`SELECT a.id::text,a.entity_type AS "entityType",a.record_id::text AS "recordId",a.file_name AS "fileName",
    ...
    LEFT JOIN users u ON u.id=a.uploaded_by ORDER BY a.created_at DESC`);
  return result.rows;
}

const allowedUploadTypes = new Set(["application/pdf", "image/png", "image/jpeg", "text/plain", "text/csv"]);

export async function saveAttachment(input: { entityType: "request" | "invoice" | "delivery"; recordId: string; file: File }, actor: { id: string; name: string }) {
  ...
  await withAuditTransaction({ userId: actor.id }, (client) => client.query(`INSERT INTO attachments
    (id,entity_type,record_id,file_name,content_type,storage_path,uploaded_by) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
    [id, input.entityType, input.recordId, safeName, input.file.type, relativePath, actor.id]));
}

export async function loadAttachmentFile(id: string) {
  if (isDemoMode()) return null;
  const result = await query<{ fileName: string; contentType: string; storagePath: string }>(`SELECT file_name AS "fileName",content_type AS "contentType",storage_path AS "storagePath" FROM attachments WHERE id=$1`, [id]);
