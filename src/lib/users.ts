...
function demoUsers() {
  if (!global.__axoraDemoUsers) global.__axoraDemoUsers = [{ id: "demo-admin", email: process.env.DEMO_EMAIL || "demo@axora.local",
    displayName: "Axora demo administrator", role: "ADMIN", active: true, createdAt: new Date().toISOString() }];
...
export async function listUsers(): Promise<UserRecord[]> {
  ...
  const result = await query<UserRecord>(`SELECT u.id::text,u.email,u.display_name AS "displayName",r.role_key AS role,u.active,
    u.last_login_at::text AS "lastLoginAt",u.created_at::text AS "createdAt" FROM users u JOIN roles r ON r.id=u.role_id ORDER BY u.display_name`);
  return result.rows;
}
...
export async function createUser(input: { email: string; displayName: string; role: UserRole; password: string }, actorId: string) {
  ...
  if (isDemoMode()) {
    ...
    demoUsers().push({ id: randomUUID(), email: input.email.toLowerCase(), displayName: input.displayName, role: input.role, active: true, createdAt: new Date().toISOString() });
    return;
  }
...
export async function setUserActive(id: string, active: boolean, actorId: string) {
  ...
  await withAuditTransaction({ userId: actorId, reason: active ? "Account activated" : "Account deactivated" },
    (client) => client.query("UPDATE users SET active=$2 WHERE id=$1", [id, active]));
}
