...
export async function assignDeliveryDriver(actor: SessionUser, input: {
  ...
  if (isDemoMode()) return;
  await withAuditTransaction({ actor, reason: "Assigned Delivery Agent" }, async (client) => {
    ...
    `, [input.driverUserId]);
    if (!activeDriver.rowCount) throw new Error("Choose an active Delivery Agent.");
    ...
    const event = await appendWorkflowEvent(client, {
      ...
      actor,
      newState: "Delivery Agent assigned",
      source: "WEB",
