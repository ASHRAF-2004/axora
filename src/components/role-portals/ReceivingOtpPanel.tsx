...
export function ReceivingOtpPanel() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      void refresh().catch(() => setError("Delivery confirmation unavailable"));
    }, 0);
  ...
  const issue = async (jobId: string) => {
    ...
    if (!response.ok) { setError("Delivery confirmation unavailable"); return; }
  ...
  return <section className={styles.shell} aria-label={copy.receiverTitle}>
    ...
    <div className={styles.jobList}>{jobs.filter((job) => job.proofPolicy.includes("OTP")).map((job) => <article className={styles.job} key={job.id}><header className={styles.jobHeader}><div><p>{job.requestNumber} · {job.branchName}</p><h2>{job.code}</h2></div><span className={styles.state}>{job.status.replaceAll("_", " ")}</span></header><div className={styles.jobBody}><p>{job.scheduledLocalStart?.replace("T", " ")} · {job.destinationTimezone}</p><button className={styles.actionButton} data-primary="true" type="button" onClick={() => void issue(job.id)}>{copy.generateCode}</button></div></article>)}</div>
  </section>;
}
