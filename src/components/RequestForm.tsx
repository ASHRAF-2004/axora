  ...
  return (
    <form action={createRequestAction} className="panel form-panel" style={{ position: "static" }}>
      <div className="form-grid">
        ...
        <label>Branch<select name="branchId" required defaultValue="" disabled={!companyId}><option value="" disabled>{companyId ? "Select branch" : "Choose company first"}</option>{availableBranches.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.name}</option>)}</select></label>
      ...
      <div className="line-builder">{items.map((key, index) => (
        <div className="line-card" key={key}>
          <label>Product<select name="productId" required defaultValue=""><option value="" disabled>Select product</option>{products.filter((item) => item.status === "Active").map((item) => <option key={item.id} value={item.id}>{item.code} · {item.name}</option>)}</select></label>
