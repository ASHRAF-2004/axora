  ...
  it("installs least-privilege staff preferences, driver claim, recovery and deletion capabilities", async () => {
    ...
    try {
      ...
      const result = await db.query<{
        ...
        claim_execute: boolean; release_execute: boolean; deletion_execute: boolean;
        ...
        SELECT
          ...
          has_function_privilege('axora_app','axora_delete_or_archive_company(uuid,uuid,uuid,text,text,timestamptz)','EXECUTE') AS deletion_execute,
      ...
      `);
      expect(result.rows[0]).toEqual({ preference_rls: true, tombstone_rls: true, direct_profile: false, claim_execute: true, release_execute: true, deletion_execute: true, direct_operational_check: false, customer_reference: true });
    } finally {
      await db.close();
    }
  }, 30_000);
