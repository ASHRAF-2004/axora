...
const turnstileDeviceHash = "e".repeat(64);
...
describe("public visitor-choice endpoint", () => {
  beforeEach(() => {
    ...
    mocks.buildIdentity.mockImplementation((input: { ephemeralId?: string }) => ({
      tokenHash,
      networkHash,
      ...(input.ephemeralId ? { turnstileDeviceHash } : {}),
    }));
  ...
  it("verifies and atomically records one same-origin choice", async () => {
    ...
    expect(mocks.claim).toHaveBeenCalledWith(expect.objectContaining({
      identity: expect.objectContaining({ tokenHash, networkHash, turnstileDeviceHash }),
      choice: "EARLY_BIRD",
      locale: "en",
