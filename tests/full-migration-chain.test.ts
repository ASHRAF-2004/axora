...
describe("complete forward migration chain", () => {
  it("applies every numbered migration through 136 to an empty database", async () => {
    const db = new PGlite();
    try {
      const available = await migrationFiles();
      expect(available.slice(-90)).toEqual([
        ...
        "097_current_invitation_scope_enforcement.sql",
        "098_role_template_permission_selection.sql",
        "099_unified_user_appearance.sql",
        "100_role_scoped_user_creation_resend_consolidation.sql",
        "101_existing_user_access_management.sql",
        "102_company_acquisition_activation_hardening.sql",
        "103_operating_model_company_and_location.sql",
        "104_company_wallet_and_approve_pay.sql",
        "105_procurement_finance_core.sql",
        ...
        "112_company_activation_contract_reconciliation.sql",
        "113_company_admin_branch_location_budget_foundation.sql",
        "114_company_admin_shopping_cart_contract.sql",
        "115_company_admin_direct_purchase.sql",
        "116_delivery_agent_e2e.sql",
        "117_company_delivery_tracking_visibility.sql",
        "118_delivery_self_claim_acceptance_window.sql",
        "119_cam_received_quantity_authorization.sql",
        "120_cam_customer_invoice_access.sql",
        "121_e164_phone_write_boundary.sql",
        "122_cam_commercial_confidentiality_ceiling.sql",
        "123_public_contact_enquiry_contract.sql",
        "124_requests_canonical_order_workspace.sql",
        ...
        "129_integration_webhook_platform.sql",
        "130_cleanup_worker_grant_reconciliation.sql",
        "131_integration_webhook_credential_delivery.sql",
        "132_slack_native_integration.sql",
        "133_supervisor_demo_hardening.sql",
        "134_budget_refresh_and_dashboard_consistency.sql",
  ...
  it("upgrades an existing 109 schema to 110 without changing account data", async () => {
    const db = new PGlite();
    try {
      await db.exec("CREATE ROLE axora_app NOLOGIN");
      await applyMigrations(db, { through: "109_axora_email_usage_tracking.sql" });
      ...
      await db.exec(await readFile(
        migrationUrl("110_user_creation_first_attempt_reliability.sql"),
        "utf8",
      ));
      const after = await db.query<{
        users: number;
        invitations: number;
        ownerCreationOverload: string | null;
        lifecycleSyncOverload: string | null;
      }>(`
        SELECT
          (SELECT count(*)::int FROM users) AS users,
          (SELECT count(*)::int FROM account_setup_invitations) AS invitations,
          to_regprocedure(
            'public.axora_lock_user_creation_scope(uuid,uuid,integer,text,text,uuid,uuid,uuid,uuid,timestamptz)'
          )::text AS "ownerCreationOverload",
          to_regprocedure(
            'public.axora_sync_company_administrator(uuid,uuid,integer,uuid,text,timestamptz)'
          )::text AS "lifecycleSyncOverload"
      `);
