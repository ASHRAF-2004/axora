...
migration_path = Path("database/migrations/041_delegated_access_management.sql")
migration = migration_path.read_text()
migration = replace_once(
    migration,
    "  grantee_role_key text;\n  grantee_is_owner boolean;",
    "  grantee_role_key text;\n  grantee_scope_type text;\n  grantee_is_owner boolean;",
    "grantee scope declaration",
)
migration = replace_once(
    migration,
    ...
    "  INTO grantee_account_kind,grantee_is_owner,grantee_role_key,\n"
    "    grantee_scope_type",
    "grantee assignment query",
)

helper_anchor = "CREATE OR REPLACE FUNCTION public.axora_delegation_authority_is_live("
active_helper = dedent("""\
...
migration = replace_once(
    migration,
    helper_anchor,
    active_helper + helper_anchor,
    "active delegated scope helper",
)
...
old_start = "    IF grantee_account_kind='COMPANY' THEN\n"
old_end = (
...
start = migration.find(
    old_start,
    migration.find("FOR scope_value IN", migration.find("axora_create_delegated_access")),
)
end = migration.find(old_end, start)
if start < 0 or end < 0:
    raise SystemExit("delegated grantee compatibility block was not found")
...
"""), "    ")
migration = migration[:start] + new_block + migration[end:]
...
migration = replace_once(
    ...
    ") FROM PUBLIC;\n" + public_revoke,
    "public delegated helper revoke",
)
app_revoke = (
    "    REVOKE ALL ON FUNCTION public.axora_delegation_authority_is_live(\n"
    "      uuid,timestamptz\n"
...
migration = replace_once(
    ...
    "      text,uuid,uuid,uuid,uuid\n"
    "    ) FROM axora_app;\n" + app_revoke,
    "application delegated helper revoke",
)
migration_path.write_text(migration)

grants_path = Path("database/admin/apply-app-grants.sql")
grants = grants_path.read_text()
revoke_old = (
...
grants = replace_once(grants, revoke_old, revoke_new, "deployment grant revocation")
grant_old = (
    "  public.axora_remove_approval_limit(uuid,uuid,uuid,text)\n"
    "TO axora_app;"
)
...
grants_path.write_text(replace_once(
    grants,
    grant_old,
    grant_new,
    "deployment command grant",
))
...
list_anchor = '        "040_approval_limit_management.sql",\n'
full = replace_once(
    full,
    list_anchor,
...
full_path.write_text(replace_once(
    full,
    apply_anchor,
    apply_replacement,
...
for path in [
    ...
    file = Path(path)
    text = file.read_text()
    old = '"040_approval_limit_management.sql"'
    if text.count(old) != 1:
        raise SystemExit(f"{path}: expected one latest migration expectation")
    file.write_text(text.replace(old, '"041_delegated_access_management.sql"'))

fixture_path = Path("tests/delegated-access-management-migration.test.ts")
fixture = fixture_path.read_text()
users_start = fixture.index("  await db.query(`\n    INSERT INTO users(")
users_end = fixture.index(
    "  await db.query(`\n    INSERT INTO company_memberships(",
    users_start,
)
corrected_users = indent(dedent("""\
await db.query(`
  INSERT INTO users(
...
"""), "  ")
fixture = fixture[:users_start] + corrected_users + fixture[users_end:]

roles_start = fixture.index("  await db.query(`\n    INSERT INTO role_assignments(")
roles_end = fixture.index(
    "  await db.query(`\n    INSERT INTO user_sessions(",
    roles_start,
)
corrected_roles = indent(dedent("""\
await db.query(`
  INSERT INTO role_assignments(
...
"""), "  ")
fixture = fixture[:roles_start] + corrected_roles + fixture[roles_end:]

deny_anchor = (
    "      await db.query(`\n"
    "        INSERT INTO user_permission_overrides("
)
inactive_check = indent(dedent("""\
await db.query(
...
fixture = replace_once(
    ...
    deny_anchor,
    inactive_check,
    "delegation inactive-scope test",
)
fixture = replace_once(
    fixture,
    ...
    "        authorityHelper: boolean;\n"
    "      }>(`",
    "delegation privilege result type",
)
privilege_query = (
...
fixture = replace_once(
    ...
    "        authorityHelper: false,\n"
    "      });",
    "delegation privilege expectation",
)
fixture = replace_once(
    fixture,
    ...
    '      expect(source).toContain("public.axora_delegation_authority_is_live(");',
    "delegation helper deployment assertion",
)
