...
COMPANY_EN = (
    ...
    t("ROLES & SCOPE", "Give people only the access they need", "Roles control actions. Company and branch scope control which records a person may see.", [
        ...
        ("Approvers", "Branch and Company Approvers make independent decisions within their scope and approval limit."),
    ...
    ], "shop", ("Search", "Cart", "Explain need", "Submit"), "CUSTOMER BOUNDARY", "Company users do not create catalog products or see Axora buying cost, margin or private operational details."),
    ...
    ], "approval", ("Submitted", "Checked", "Decision", "Audit event"), "APPROVAL MEANS", "Approval authorizes company budget; the customer can then Pay and Axora can fulfil the request."),
    t("TRACKING", "Follow the complete request timeline", "The current status and append-only events explain what happened, when and by whom.", [
        ...
        ("Use the timeline", "Follow approval, payment, invoice, buying, delivery, receipt and completion events."),
    ...
    ], "timeline", ("Request", "Approval", "Pay", "Invoice", "Delivery", "Receipt"), "PRIVACY", "You see only timeline details allowed by your tenant, role and branch scope."),
    t("DELIVERY", "Buy and deliver through one mobile workflow", "The assigned Delivery Guy uses a focused phone portal; the branch keeps independent receiving authority.", [
        ("Assignment", "The Delivery Guy sees today’s assigned paid request, approved address, contact, product quantities and instructions."),
        ("Buying and journey events", "Accept, start buying, confirm items bought, travel, arrive, partially deliver or report an issue."),
        ("Delivery evidence", "Upload permitted proof and record receiver name, quantity, damage or missing items."),
        ("Weak network", "Queued events remain visible until synchronized; an event must never disappear silently."),
    ], "driver", ("Assigned", "Buying", "Out for delivery", "Completed"), "LIMITED ACCESS", "The Delivery Guy cannot see budgets, internal financials, user lists or unrelated requests."),
    t("RECEIVING", "Confirm receipt independently", "Driver evidence is not final acceptance. An authorized customer recipient records inspection.", [
        ...
        ("Complete receipt", "Confirm date, receiver identity and final quantities only after inspection so Axora can complete the request."),
...
OWNER_EN = (
    t("OWNER ACCESS", "Operate through authorized owner accounts", "Authorized owners use separate Platform Owner accounts; a person's name never defines authorization.", [
        ...
        ("Protect elevated work", "Use the smallest role possible; Human Resources Management, Agents and Delivery Guys have separate duties."),
    ...
    ], "roles", ("Identity", "Membership", "Role + scope", "Invitation"), "CANONICAL ROLES", "Use Platform Owner, Human Resources Management, Client Account Manager, Company Administrator, Branch or Department Administrator, Approver, Requester and Delivery Guy."),
    t("OWNER NAVIGATION", "Run operations from the top shell", "Frequent modules stay in top navigation; system tools and settings stay in the hamburger drawer.", [
        ("Top work", "Use Dashboard, Companies, Catalog, Deliveries and Invoices when permissions allow."),
    ...
    t("CATALOG", "Publish recognizable products", "Axora owns the global catalog and keeps private buying data away from customer roles.", [
        ...
        ("Add customer details", "Use a clear name, category, unit, lead time, customer price and concise description."),
    ...
    ], "catalog", ("Validate", "Describe", "Upload image", "Publish"), "PRIVATE DATA", "Buying cost, margin and internal notes never appear in the company Shop."),
    t("TRACKING & NOTIFICATIONS", "Use one end-to-end event model", "Append-only workflow events preserve history beyond the current row status.", [
        ("Correlate the workflow", "Use one workflow ID across request, approval, payment, invoice, buying, delivery and receipt."),
    ...
    ], "timeline", ("Need", "Approval", "Pay", "Buying", "Delivery", "Receipt"), "IMMUTABLE HISTORY", "Never reconstruct critical history only from the current record state."),
    ...
    ], "driver", ("Assign", "Buy", "Deliver", "Recipient proof"), "MINIMUM DISCLOSURE", "Do not expose budgets, invoices, internal costs, user lists or unrelated customers to a Delivery Guy."),
    t("RECEIVING", "Keep inspection independent", "Customer recipient confirmation establishes what the company accepted, not merely what the Delivery Guy reported.", [
    ...
    t("FINANCE", "Oversee payment and invoice finalization", "The trusted paid event finalizes the customer invoice while buying and delivery remain separate.", [
        ...
        ("Inspect delivery independently", "Receipt evidence and delivery discrepancies remain separate from the Delivery Guy's proof."),
    ...
    t("SECURITY & AUDIT", "Protect every tenant and sensitive action", "Authorization, uploads, authentication, email and support operations require defense in depth.", [
        ("Enforce tenant boundaries", "Verify role and scope server-side and keep internal financial fields out of customer responses."),
    ...
    t("OPERATIONS CHECKLIST", "Verify the complete path before launch", "A production-ready tenant proves its controls with low-risk records before normal purchasing.", [
        ...
        ("Delivery and finance", "Delivery evidence, independent receipt, invoice and payment status remain auditable."),
...
OWNER_AR = tuple(ar_topic(en, *args) for en, args in zip(OWNER_EN, (
    ...
    ("التتبع والتنبيهات", "استخدم نموذج أحداث واحداً", "تحفظ الأحداث الملحقة التاريخ أبعد من الحالة الحالية.", [("اربط المسار", "استخدم معرفاً واحداً للطلب والاعتماد والشراء والتسليم والاستلام والفاتورة."), ("سجل بيانات آمنة", "المنفذ والدور والمستأجر والفرع والوقت والحالتان والسبب والمصدر."), ("نبه التالي", "استخدم رسائل داخلية وبريدية غير مكررة بروابط آمنة."), ("راقب التأخير", "استخدم إنذارات المدة والاستثناء دون كشف بيانات خاصة.")], "تاريخ ثابت", "لا تبن التاريخ الحرج من حالة الصف الحالية فقط.", ("احتياج", "اعتماد", "توريد", "تسليم", "استلام", "مالية")),
    ("عمليات التسليم", "اسند السائق بأقل بيانات", "تعرض بوابة الهاتف معلومات المهمة المسندة فقط.", [("أنشئ المهمة", "حدد التسليم والسائق وعنوان الفرع والنافذة والتعليمات."), ("راقب الحالة", "تابع القبول والبدء والوصول والمحاولة والجزئي والكامل والفشل."), ("احم الدليل", "تحقق من الملفات وافصل دليل السائق عن استلام العميل."), ("عالج عدم الاتصال", "استخدم مزامنة متكررة آمنة وأظهر الأحداث المنتظرة أو الفاشلة.")], "أقل كشف", "لا تكشف الميزانية أو الفواتير أو التكاليف الداخلية أو المستخدمين أو العملاء الآخرين للسائق.", ("إسناد", "رحلة", "دليل", "استلام")),
    ...
    ("المالية", "أشرف على الدفع وإصدار الفاتورة", "يحوّل حدث الدفع الموثوق لقطة الطلب المعتمدة إلى فاتورة عميل نهائية مستقلة عن التسليم.", [("تحقق من اللقطة", "استخدم المنتجات والكميات والأسعار والرسوم والعملة المعتمدة."), ("احم التكرار", "ينتج الطلب دفعة واحدة ورقم فاتورة واحداً وملف PDF واحداً ورسالة منطقية واحدة."), ("افصل التسليم", "تبقى الشراء والاستلام في مسار التسوية الخاص بها."), ("عالج فشل البريد", "أعد محاولة إرسال الفاتورة دون تغيير حالة الدفع أو الفاتورة النهائية.")], "امتداد مستقبلي", "يمكن لبوابة موثقة لاحقاً إنتاج حدث الدفع نفسه دون إعادة كتابة إصدار الفاتورة.", ("لقطة معتمدة", "مدفوع", "فاتورة نهائية", "دورة البريد")),
    ("الأمان والتدقيق", "احم كل مستأجر وإجراء حساس", "يحتاج التفويض والرفع والمصادقة والبريد والدعم إلى دفاع متعدد.", [("اعزل المستأجر", "تحقق من الدور والنطاق وافصل الحقول المالية الداخلية وحقول العميل."), ("احم المصادقة", "جزئ كلمات المرور بخوارزمية بطيئة والرموز عالية العشوائية ودوّر الجلسات."), ("احم الملفات", "تحقق من النوع والمحتوى والاسم والانتهاء والحدود والرؤوس."), ("دقق الارتفاع", "سجل تغييرات الوصول والهوية والتسليم والمالية والدعم دون أسرار.")], "سلامة البريد", "استخدم نطاق أكسورا الموثق وتوقيع الويب هوك وقائمة المنع وعنوان رد مراقباً.", ("مصادقة", "تفويض", "تحقق", "تدقيق")),
...
MANUALS = (
    Manual(FILES[0], "en", "company", "Company procurement manual", "From secure invitation to receiving and completion", "For Company Administrators, Branch Administrators, Approvers, Requesters, Receivers and Auditors", "Your company controls people, branches, budgets, requests, approvals, Pay and receiving. Axora controls the global catalog and delivery fulfilment.", COMPANY_EN),
    Manual(FILES[1], "ar", "company", "دليل مشتريات الشركة", "من الدعوة الآمنة إلى الاستلام والتسوية", "لمديري الشركة والفروع والمعتمدين ومقدمي الطلبات والمعتمدين ومقدمي الطلبات ومسؤولي التوصيل", "تتحكم شركتك في الأشخاص والفروع والميزانيات والطلبات والاعتمادات والاستلام، وتتحكم أكسورا في الكتالوج العام والشراء والتنفيذ والتسليم.", COMPANY_AR),
    Manual(FILES[2], "en", "owner", "Axora platform owner manual", "Tenant onboarding, catalog, delivery, finance and security", "For authorized Axora Platform Owners", "Axora owns platform governance, tenant onboarding, the global catalog, delivery operations, finance controls, audit and recovery.", OWNER_EN),
    Manual(FILES[3], "ar", "owner", "دليل مالك منصة أكسورا", "تهيئة الشركات والكتالوج والشراء والتسليم والمالية والأمان", "لمالكي منصة أكسورا المخولين", "تملك أكسورا حوكمة المنصة وتهيئة المستأجرين والكتالوج العام والتكاليف الداخلية والشراء والتسليم وضوابط المالية والتدقيق والتعافي.", OWNER_AR),
)
...
def draw_visual(c, kind, x, y, w, h, is_rtl):
    ...
    labels = {
        ...
        "timeline": ("Request", "Approval", "Pay", "Buying", "Delivery", "Receipt"),
    ...
    if is_rtl:
        labels = tuple({
            ...
            "Approval":"اعتماد", "Pay":"دفع", "Buying":"شراء", "Delivery":"تسليم", "Receipt":"استلام", "Finance":"مالية",
