...
restore_list="$partial_dir/database.restore-list.txt"
restore_list_filtered="$partial_dir/database.restore-list.filtered.txt"
if [[ -n "${AXORA_TEST_DOCKER_LOG:-}" ]]; then
  ...
  docker exec -i "$db_container" pg_restore --list < "$partial_dir/database.dump" > "$restore_list"
